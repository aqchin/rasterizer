...
// sP(left, right, top, bottom, near, far) (04_Projection2 16/51)
// sP(fov, aspect, near, far)
//void Matrix4::setProjection(double l, double r, double t, double b, double n, double f) 
void Matrix4::setProjection(double fov, double a, double n, double f) {
	identity();
	double t = tan(fov * M_PI /2 /180); // to radians
	this->m[0][0] = 1 / (a * t);
	this->m[1][1] = 1 / t;
	this->m[2][2] = (n + f) / (n - f);
	this->m[2][3] = 2 * n * f / (n - f);
	this->m[3][2] = -1;
	this->m[3][3] = 0;
	/*
	this->m[0][0] = 2 * n / (r - l);
	...
	this->m[1][1] = 2 * n / (t - b);
	this->m[1][2] = (t + b) / (t - b);
	this->m[2][2] = -1 * (f + n) / (f - n);
	this->m[2][3] = -2 * f * n / (f - n);
	this->m[3][2] = -1;
	*/
}

// D(x0, x1, y0, y1) (04_Projection2 19/51)
void Matrix4::setViewport(double x0, double x1, double y0, double y1) {
	identity();
	this->m[0][0] = (x1 - x0) / 2;
	this->m[0][3] = (x0 + x1) / 2;
	this->m[1][1] = (y1 - y0) / 2;
	this->m[1][3] = (y0 + y1) / 2;
	this->m[2][2] = this->m[2][3] = 0.5;
}